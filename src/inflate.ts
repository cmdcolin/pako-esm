...
export class Inflate {
  options: Required<Pick<InflateOptions, 'chunkSize' | 'windowBits' | 'to'>> &
    InflateOptions
  err: number
  msg: string
  ended: boolean
  ...
  constructor(options?: InflateOptions) {
    ...
    if (opt.dictionary) {
      ...
      if (opt.raw) {
        ...
        status = inflateSetDictionary(this.strm, opt.dictionary as Uint8Array)
        if (status !== Z_OK) {
          throw new Error(msg[status]!)
        }
      }
   ...
   * ```
   **/
  push(data: Uint8Array | ArrayBuffer | string, mode?: number | boolean) {
    var strm = this.strm
    ...
    do {
      ...
      if (status === Z_NEED_DICT && dictionary) {
        ...
        if (typeof dictionary === 'string') {
          ...
          dict = new Uint8Array(dictionary as ArrayBuffer)
        } else {
          dict = dictionary
        }

        status = inflateSetDictionary(this.strm, dict as Uint8Array)
      }
  ...
  onEnd(status: number) {
    ...
    if (status === Z_OK) {
      if (this.options.to === 'string') {
        ...
        this.result = (this.chunks as string[]).join('')
      } else {
